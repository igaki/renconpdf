...
def getname_fromfile(img_):
    ...
    img_rpd = re.sub(r'\([0-9]+\)|\s|　', '', img_)  # 半角スペース，全角スペースと数字を削除
    under = img_rpd.rfind('-') + 1  # 末尾からみた-の次の文字のindex
...
if __name__ == "__main__":
    ...
    print(df)
    # 機種依存文字をファイル名に使われているとcp932で書き込めなくなるのでutf8で出力．ただしExcelで開くと文字化けする
    df.to_csv(sys.argv[2]+".csv", encoding='utf8')
